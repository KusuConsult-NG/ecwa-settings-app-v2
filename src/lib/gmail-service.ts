...
export async function sendInviteEmail(data: InviteEmailData): Promise<boolean> {
  try {
    ...
    const mailOptions = {
      from: `"ChurchFlow Team" <${process.env.GMAIL_USER}>`,
      to: data.to,
    ...
    console.log('📧 Invite email sent successfully via Gmail to:', data.to)
    return true
  } catch (error) {
    console.error('❌ Error sending invite email via Gmail:', error)
    return false
  }
}
...
export async function sendWelcomeEmail(data: WelcomeEmailData): Promise<boolean> {
  try {
    // If no Gmail credentials, log to console for development
    if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
      console.log('📧 WELCOME EMAIL (Development Mode - Gmail not configured):')
      console.log('To:', data.to)
    ...
    await transporter.sendMail(mailOptions)
    console.log('📧 Welcome email sent successfully via Gmail to:', data.to)
    return true
  } catch (error) {
